/* main.c -- KKR .so loader
 *
 * Copyright (C) 2025 Rocroverss
...
SDL_RWops *SDL_RWFromFile_hook(const char *fname, const char *mode) {
	SDL_RWops *f;
	char real_fname[256];
	sceClibPrintf("SDL_RWFromFile(%s,%s)\n", fname, mode);
	if (strncmp(fname, "ux0:", 4)) {
		sprintf(real_fname, "%s/%s", data_path, fname);
		sceClibPrintf("SDL_RWFromFile patched to %s\n", real_fname);
...
SDL_Window *SDL_CreateWindow_hook(const char *title, int x, int y, int w, int h, Uint32 flags) {
	return SDL_CreateWindow("kkr", 0, 0, SCREEN_W, SCREEN_H, flags);
}
...
void _assert2(const char *file, int line, const char *func, const char *expr) {
	sceClibPrintf("assertion failed:\n%s:%d (%s): %s\n", file, line, func, expr); 
	sceKernelExitProcess(0); 
}


static so_default_dynlib default_dynlib[] = {
	//small crash on shader:
	{ "__assert2", (uintptr_t)&_assert2 },
...
int main(int argc, char *argv[]) {
	...
	char fname[256];
	sprintf(data_path, "ux0:data/kkr");
